...
STATE *TAG::CreateStartState() const {
  ...
  for (int i = 0; i < NumOpponents; ++i)
    tagstate->OpponentPos.push_back(
        GetCoord(SimpleRNG::ins().Random(NumCells)));
  return tagstate;
}
...
bool TAG::Step(STATE &state, int action, int &observation,
               double &reward) const {
  TAG_STATE &tagstate = safe_cast<TAG_STATE &>(state);
  ...
  // Move opponents
  for (int opp = 0; opp < NumOpponents; ++opp)
    if (IsAlive(tagstate, opp)) MoveOpponent(tagstate, opp);

  // Move action
  if (action < 4) {
    reward = -1;
    COORD nextpos = tagstate.AgentPos + coord::Compass[action];
    if (Inside(nextpos)) tagstate.AgentPos = nextpos;
  }
...
int TAG::GetObservation(const TAG_STATE &tagstate, int action) const {
  ...
  if (action < 4)
    for (int opp = 0; opp < NumOpponents; ++opp)
      if (tagstate.OpponentPos[opp] == tagstate.AgentPos) obs = NumCells;
  return obs;
}
...
bool TAG::LocalMove(STATE &state, const HISTORY &history, int /*stepObs*/) const {
  ...
  if (!IsAlive(tagstate, opp)) return false;
...
void TAG::DisplayBeliefs(const BELIEF_STATE &, std::ostream &) const {}
...
void TAG::DisplayState(const STATE &state, std::ostream &ostr) const {
  ...
  cgrid.SetAllValues('.');
  for (int opp = 0; opp < NumOpponents; ++opp)
    if (IsAlive(tagstate, opp)) cgrid(tagstate.OpponentPos[opp]) = '@';
...
void TAG::DisplayObservation(const STATE &, int observation,
