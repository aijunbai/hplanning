...
void ROCKSAMPLE::Init_7_8() {
  ...
  COORD rocks[] = {COORD(2, 0), COORD(0, 1), COORD(3, 1), COORD(6, 3),
                   COORD(2, 4), COORD(3, 4), COORD(5, 5), COORD(1, 6)};
...
void ROCKSAMPLE::Init_11_11() {
  ...
  COORD rocks[] = {COORD(0, 3), COORD(0, 7), COORD(1, 8), COORD(2, 4),
                   COORD(3, 3), COORD(3, 8), COORD(4, 3), COORD(5, 8),
                   COORD(6, 1), COORD(9, 3), COORD(9, 9)};
                      ...
                      double &reward) const
{
  ...
  if (action > E_SAMPLE)  // check
  {
    ...
    double distance =
        coord::EuclideanDistance(rockstate.AgentPos, RockPos[rock]);
    ...
    rockstate.Rocks[rock].ProbValuable =
...
bool FieldVisionRockSample::Step(STATE &state, int action, int &observation,
                                 double &reward) const {
  int tmp_obs;
  double tmp_reward;
