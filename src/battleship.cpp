  ...
  for (int length = MaxLength; length >= 2; --length) {
    ...
    for (int shipIndex = 0; shipIndex < numShips; ++shipIndex) {
      ...
      do {
        ...
        ship.Position = COORD(SimpleRNG::ins().Random(XSize),
...
bool BATTLESHIP::Step(STATE &state, int action, int &observation,
                      double &reward) const {
  BATTLESHIP_STATE &bsstate = safe_cast<BATTLESHIP_STATE &>(state);
...
bool BATTLESHIP::SwitchThreeShips(BATTLESHIP_STATE &bsstate) const {
  ...
  int shortOffset2 = SimpleRNG::ins().Random(
      shortOffset1 + 2, longShip.Length - shortShip2.Length + 1);
...
void BATTLESHIP::DisplayBeliefs(const BELIEF_STATE &beliefState,
  ...
  for (int i = 0; i < beliefState.GetNumSamples(); i++) {
    const BATTLESHIP_STATE *bsstate =
        safe_cast<const BATTLESHIP_STATE *>(beliefState.GetSample(i));
...
void BATTLESHIP::DisplayObservation(const STATE &, int observation,
                                    ostream &ostr) const {
  if (observation)
    ostr << "o=Hit\n";
  else
