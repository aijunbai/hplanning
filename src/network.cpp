...
void NETWORK::Make3LegsNeighbours() {
  ...
  for (int i = 1; i < NumMachines; ++i) {
    if (i < NumMachines - 3) Neighbours[i].push_back(i + 3);
    if (i <= 4)
...
STATE *NETWORK::CreateStartState() const {
  ...
  for (int i = 0; i < NumMachines; i++) nstate->Machines.push_back(true);
  return nstate;
}
...
bool NETWORK::Step(STATE &state, int action, int &observation,
                   double &reward) const {
  NETWORK_STATE &nstate = safe_cast<NETWORK_STATE &>(state);
  reward = 0;
  observation = 2;
  ...
  for (int i = 0; i < NumMachines; i++)
    for (uint j = 0; j < Neighbours[i].size(); ++j)
      if (!nstate.Machines[Neighbours[i][j]]) neighbourFailure[i] = true;
...
void NETWORK::DisplayBeliefs(const BELIEF_STATE &, std::ostream &) const {}

void NETWORK::DisplayState(const STATE &state, std::ostream &ostr) const {
  const NETWORK_STATE &nstate = safe_cast<const NETWORK_STATE &>(state);
  for (int i = 0; i < NumMachines; i++)
    ostr << i << ": " << (nstate.Machines[i] ? "operational" : "failed")
         << endl;
}

void NETWORK::DisplayObservation(const STATE &, int observation,
...
void NETWORK::DisplayAction(int action, std::ostream &ostr) const {
  switch (action) {
    if (action == NumMachines) ostr << "No action" << endl;
    int machine = action / 2;
    int reboot = action % 2;
